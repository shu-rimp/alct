...
```
POST /ocr (PNG bytes)
        → token auth (X-ALCT-Token 헤더 검증, ALCT_SERVER_TOKEN 미설정 시 스킵(사용자 셀프호스팅용))
...
- Token auth: `X-ALCT-Token` 헤더 검증 (`ALCT_SERVER_TOKEN` env 미설정 시 스킵)
- Rate limit: IP당 30req/60s (커스텀 `_requestTimestamps` dict, x-forwarded-for 지원)
- 토큰 불일치 → 403, 빈 body → 400, rate limit 초과 → 429

### Deployment
- 환경변수는 서버 머신의 `.env` (gitignored) 로 주입
