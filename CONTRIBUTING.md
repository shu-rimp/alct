...
일반 번역기로 번역되지 않는 고유 용어(캐릭터·무기·아이템·스킬 명 등)를 미리 한국어로 바꿔주는 사전입니다. 현재는 **Apex Legends** 용어 위주로 구성되어 있으며, 다른 게임으로의 확장은 검토 중입니다. 깃허브 계정이 있는 사용자라면 **누구나** 참여할 수 있습니다.

기여해주신 용어집은 머지 후 모든 사용자에게 제공됩니다(별도의 업데이트 없이 앱 시작 시 최신본으로 갱신). 용어집이 완성될 수록 모두가 더 나은 번역 서비스를 이용할 수 있습니다. 
...
- **인젝션·합성 입력 금지** - 다른 프로세스(특히 안티치트가 적용되는 게임 프로세스)에 대한 메모리 읽기/쓰기, DLL 인젝션, 렌더링 후킹 및 마우스/키보드 합성 입력 주입(`SendInput`)은 도입하지 마세요. UI Automation 등 OS가 공개하는 접근성 API를 통한 읽기는 무방합니다.
