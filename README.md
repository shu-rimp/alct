...
단축키를 누르면 채팅 영역을 캡처해 번역 결과를 오버레이로 표시합니다.
...
- **OCR 릴레이 서버** 는 오픈소스 RapidOCR로 텍스트만 추출하고, 이미지는 즉시 폐기합니다. **현재 서버 사양이 낮아 사용자가 늘면 채팅 번역이 느려질 수 있습니다.** (음성·입력 번역은 서버를 거치지 않아 무관하게 사용할 수 있습니다.) 
  > 셀프 호스팅을 원하면 [서버 저장소](https://github.com/shu-rimp/alct-server#셀프-호스팅-self-hosting)를 참고하세요.
...
버그 제보, 기능 제안, 코드 기여 모두 환영해요 — [기여 가이드](CONTRIBUTING.md)를 참고해주세요.        

가장 도움이 되는 기여는 **게임 용어집**입니다. 용어집은 [서버 저장소](https://github.com/shu-rimp/alct-server?tab=contributing-ov-file#기여-가이드)에서 관리합니다.
