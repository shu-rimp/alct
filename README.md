...
<p align="center">
  ALCT는 외국어 텍스트와 음성을 한국어로 번역해 화면 위에 띄우는 <b>실시간 번역 오버레이</b>입니다.<br/>
  번거로운 조작 없이 음성과 텍스트, 입력을 쉽고 빠르게 번역하여 일반적인 사용 환경 외 특수한 환경(예: 온라인 게임)에서도 사용하기 좋습니다. 
</p>
...
ALCT는 화면에 보이는 텍스트(또는 음성)을 읽어, 화면 위에 투명 오버레이로 한국어 번역을 띄웁니다.

**1. 가볍습니다** — 번역은 외부 번역 서비스 API에서 처리하고 텍스트 OCR은 단축키를 누른 순간에만 기기 내에서 가볍게 동작하도록 설계하여 부담을 최소화했습니다.
...
60분 연속 발화, 텍스트 15초·입력 30초마다 트리거하며 기록. GPU 사용량은 두 경우 모두 약 0% 입니다.

- **전 기능 사용 시(ALCT+Live Captions)** — 지속 발화 음성 번역 + 텍스트 번역 + 입력 번역: CPU 평균 약 2%, 메모리 약 720 MB 수준
...
**2. 쉽습니다** — 기존 작업을 방해하지 않도록 번거로운 조작 및 UI를 최소화하고, 편의성을 위한 다양한 맞춤 설정을 제공합니다. 설치 시 기능 소개 및 설정을 돕는 온보딩을 통해 자연스럽게 사용방법을 익힐 수 있습니다.
...
**3. 정확합니다** — 용어집 전처리를 통해 일반 번역기로 번역되지 않는 특수한 용어(예: 게임 캐릭터 이름)들은 미리 번역합니다. 모든 번역 엔진에 동일하게 적용됩니다. 용어집은 앱 업데이트 없이 자동으로 최신 상태로 갱신됩니다. Gemini 또는 DeepL 번역 엔진 선택 시 은어 또는 로마자 일본어 표기(예: yorosiku) 같은 까다로운 번역도 준수한 품질로 제공합니다. 
...
`실시간 음성 번역` 토글을 활성화하면 음성이 감지될 때 자동으로 한국어 자막을 생성합니다. **자연스러운 대화 단위**로 묶어 번역하며, 팟캐스트처럼 빠르고 지속적인 발화에도 안정적으로 자막을 제공합니다.
...
단축키를 누르면 텍스트를 인식해 번역 결과를 오버레이로 표시합니다. 텍스트 번역이 불가능한 사진·그림에 포함된 글자도 문제없이 번역합니다.

- **짧게 누르기** — 미리 지정된 영역을 번역합니다.
- **길게 누르기** — 드래그로 번역할 영역을 직접 지정합니다. (1회성)

<img src="https://github.com/user-attachments/assets/614161be-6748-4396-8c8e-cfb48583763d" alt="텍스트 번역 데모" width="640" />

### ⌨️ 입력 번역 `기본: Ctrl+G`

입력한 텍스트을 복사 후 단축키를 누르면 대상 언어로 번역해 클립보드에 넣어줍니다. `Ctrl+V`로 붙여넣어 사용합니다. 
...
텍스트 번역:   단축키 → 화면 캡처 → 온디바이스 OCR → 번역 API → 오버레이
...
ALCT를 온라인 게임 환경에서 사용하는 경우 아래 안티치트 관련 사항을 확인하세요.
